...
void setup() {
  ...
  for (int i = 0; i < 5; i++) 
    cars[i] = new Car( int(random(200,800)), int(random(200,800)), int(random(0,300)), random(0.1,1.5), 
...
void draw() {
  ...
  //directionalLight(128, 128, 128, 2, 3, 5);
...
void mouseDragged() {
  if (pickedCar != -1) {
    cars[pickedCar].moveCar(mouseX, mouseY);
  }
}
...
class Car {
  ...
  void moveCar(float newX, float newY) {  
    cx = newX;
    cy = newY;
